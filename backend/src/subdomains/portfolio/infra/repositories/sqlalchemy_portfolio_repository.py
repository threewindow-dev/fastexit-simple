...
class SQLAlchemyProductRepository(_BaseRepo, ProductRepository):
    ...
    async def add(self, conn: Connection, product: Product) -> Product:
        ...
        return Product(
            ...
            created_at=entity.created_at,
            domestic_beta=(
                float(entity.domestic_beta)
                if entity.domestic_beta is not None
                else None
            ),
            global_beta=(
                float(entity.global_beta) if entity.global_beta is not None else None
            ),
    ...
    async def get_all(self, conn: Connection) -> list[Product]:
        ...
        return [
            Product(
                ...
                created_at=e.created_at,
                domestic_beta=(
                    float(e.domestic_beta) if e.domestic_beta is not None else None
                ),
