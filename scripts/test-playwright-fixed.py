...
async def test_frontend(page):
    ...
    try:
        ...
        # Next.js 애플리케이션 확인
        page_content = await page.content()
        if "__NEXT_DATA__" in page_content or "next" in page_content.lower():
            print("✓ Next.js application detected")
        
        # 실제 에러 확인 (500 에러 페이지, Internal Server Error)
        if "<h1>500</h1>" in page_content or "Internal Server Error" in page_content or "<h1>Application error" in page_content:
            print("✗ Page shows 500/application error")
            return False
        ...
        # 사용자 목록 섹션이 있는지 확인
        try:
            # React hydration을 위해 대기
            await asyncio.sleep(3)
            body_text = await page.text_content("body")
            if body_text and len(body_text) > 50:
                print(f"✓ Page content loaded ({len(body_text)} chars)")
                # 주요 컨텐츠 확인
                if "Users" in body_text or "Loading" in body_text or "FastExit" in body_text:
                    print("✓ Expected content found")
            else:
                print("⚠ Page content minimal")
        except Exception as e:
            print(f"⚠ Could not verify body: {e}")
